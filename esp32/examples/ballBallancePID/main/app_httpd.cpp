...
bool hsv_image = false;

HSV_t hsvValues = {82,255,103,255,0,255};
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    fb = esp_camera_fb_get();
    if (!fb) {
      ESP_LOGE(TAG,"Camera capture failed,deu");
      res = ESP_FAIL;
    } else {
      ...
      if (fb->format == PIXFORMAT_RGB565) {
        if (hsv_image){
          camera_fb_t *filtered_fb = getFilteredJPG(fb,hsvValues);
          fb = NULL;
          if (filtered_fb) {
              bool jpeg_converted = frame2jpg(filtered_fb, 80, &_jpg_buf, &_jpg_buf_len);
              if (!jpeg_converted) {
                ESP_LOGE(TAG,"JPEG compression failed");
                res = ESP_FAIL;
              }
              free(filtered_fb->buf);
              free(filtered_fb);
          } else {
              ESP_LOGE(TAG,"Conversion to framebuffer failed.");
            }
        }
        else{
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  } else if (!strcmp(variable, "Hmin")) {
    hsvValues.HMIN = val;
    res = 0;
  } else if (!strcmp(variable, "Hmax")) {
    hsvValues.HMAX = val;
    res = 0;
  } else if (!strcmp(variable, "Smin")) {
    hsvValues.SMIN = val;
    res = 0;
  } else if (!strcmp(variable, "Smax")) {
    hsvValues.SMAX = val;
    res = 0;
  } else if (!strcmp(variable, "Vmin")) {
    hsvValues.VMIN = val;
    res = 0;
  } else if (!strcmp(variable, "Vmax")) {
    hsvValues.VMAX = val;
    res = 0;
  ...
  } else if (!strcmp(variable, "hsvFilter")) {
    hsv_image = val;
    res = 0;
  }
#if CONFIG_LED_ILLUMINATOR_ENABLED
...
static void ledcAttach(int pin, uint32_t freq) {
...
void setupLedFlash(int pin) {
    ...
    ledcAttach(pin, 5000);
#else
    ESP_LOGI("LED_FLASH", "LED flash is disabled -> CONFIG_LED_ILLUMINATOR_ENABLED = 0");
#endif
}
#endif